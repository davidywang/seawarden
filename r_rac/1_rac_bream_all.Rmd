...
for (i in sites){
  ...
  dir.create(paste0(rac_folder, 'site_', i, '/')) 
...
#or
library(parallel)
mclapply(sites, FUN=function(i) rac(rac_folder, i))
```
...
for (i in sites){
  ...
  output <- paste0(feed, 'site_', i, '/Bream_population/Inputs/Forcings/Feeding.csv')
  write.table(export, file = output, row.names=FALSE, col.names = FALSE, sep=",")}
```
