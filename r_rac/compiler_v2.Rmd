...
compiler_all <- function(rac_folder, site_folder, i, h_size){
  #load csv files
  ...
  #harvest <- harvest[[1]]
  harvest <- min(which(compiled$weight > h_size))
...
sites <- list.files(paste0(bream_folder, 'output_compiled/'), pattern = '.csv')
  ...
  combined = list()
  for (i in sites){combined[[i]] <- compiler_all(bream_folder, site_folder_bream, i, 350)}
...
sites <- list.files(paste0(bass_folder, 'output_compiled/'), pattern = '.csv')
  ...
  combined = list()
  for (i in sites){combined[[i]] <- compiler_all(bass_folder, site_folder_bass, i, 450)}
