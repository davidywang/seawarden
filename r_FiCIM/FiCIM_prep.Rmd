...
#parameters
sites <-54
sim <- 7 #simulation days
dep <- -65 #water depth (meters)
bath <- 0 #0 = no bath file
cur <- 20 #increase current speed
...
c <- read.delim(paste0(current, site_id, "_current.txt"), header = FALSE, sep = "\t")
  ...
  c <- as.data.frame(cbind(d, c$V1*cur, c$V2*cur))
...
for (i in 1:index){
  ...
  #param.txt
  param$V1[2] <- sim #nrow(waste) #days
