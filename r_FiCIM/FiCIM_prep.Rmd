...
rac_folder_2 <- '/Users/Zack/0_rac/rac_bream/'
...
compiler <- function(rac_folder, site_folder, i){
  ...
  colnames(compiled) <- c("day", "Pexc", "Nexc", "Cexc", "Pwst", "Nwst", "Cwst")
...
#   compiler(rac_folder_2, site_folder_2, i)}
```
...
current <- "/Users/Zack/0_thesis_ocean_current/current_txt/"
...
write.table(c, name, sep = "\t", row.names=FALSE, col.names=FALSE)
```

```{r}
#generate txt files
for (i in 1:index){
  param$V3[1] <- site$ORIG_FID[i] #cage number
  param$V4[1] <- site$Cgx[i] #cage position
  param$V5[1] <- site$Cgy[i] #cage position
  #param$V1[3] <- 0
  #param$V2[3] <- -20

  name <- paste0("/Users/Zack/0_rac/rac_", site$Species[i], '/output_waste/', site$farm_id[i], '_total_waste.csv')
  waste <- read.csv(name, header=TRUE, sep=",", stringsAsFactors = FALSE)
  
  param$V1[1] <- nrow(waste) #days
  param$V2[1] <- site$Species.Index[i]
  ...
  write.table(param, name, sep = "\t", row.names=FALSE, col.names=FALSE)
  
  #export total_waste.txt
  name <- paste0(folder, site$farm_id[i], '_', site$ORIG_FID[i], '_total_waste_', site$Species[i], '.txt')
  write.table(waste, name, sep = "\t", row.names=FALSE, col.names=FALSE)}
```
