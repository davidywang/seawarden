...
targets <- '/Users/Zack/0_seawarden/greece/blue_bridge/farm_500m_update.shp'
...
for (i in 1:nrow(as.data.frame(targets))){
  ...
  output <- paste0(tif_output, targets$farm_id, '_bath.tif') 
...
bath_txt <- function(tif_input, i, txt_output){
  bath <- raster(paste0(tif_output, targets$farm_id, "_bath.tif"))
  ...
  output <- paste0(txt_output, targets$farm_id, '_bath.txt')
...
output <- '/Users/Zack/0_thesis_bath/bath_avg.csv'
